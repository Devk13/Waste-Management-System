...
async def create_job(db: AsyncSession, data: JobCreate) -> Job:
    # Pydantic v2: model_dump; keep exclude_unset for partials
    job = Job(**data.model_dump(exclude_unset=True))
    db.add(job)
    await db.flush()     # router usually commits
...
async def list_jobs(
    ...
    if status:
        # Accept string; coerce to enum (raises cleanly if invalid)
...
async def jobs_for_driver(
    db: AsyncSession,
    driver_id: str,
    statuses: Optional[list[JobStatus]] = None,  # optional filter
) -> List[Job]:
    stmt = (
        select(Job)
        .where(Job.assigned_driver_id == driver_id)
        .order_by(Job.window_start.nulls_last(), Job.created_at.asc())
    )
    if statuses:
        stmt = stmt.where(Job.status.in_(statuses))
    else:
        stmt = stmt.where(Job.status.in_([JobStatus.PENDING, JobStatus.IN_PROGRESS]))
    res = await db.execute(stmt)
    return res.scalars().all()
