...
async def create_job_endpoint(payload: JobCreate, db: AsyncSession = Depends(get_db)) -> JobOut:
    try:
        ...
        log.exception("create_job failed: %s", e)
        raise HTTPException(status_code=500, detail=f"create_job failed: {type(e).__name__}: {e}")
...
async def list_jobs_endpoint(
    ...
    try:
        ...
        log.exception("list_jobs failed: %s", e)
        raise HTTPException(status_code=500, detail=f"list_jobs failed: {type(e).__name__}: {e}")
...
async def patch_job_endpoint(job_id: str, payload: JobPatch, db: AsyncSession = Depends(get_db)) -> JobOut:
    try:
        ...
        result = await db.execute(
            update(Job).where(Job.id == job_id).values(**changes).returning(Job)
        )
        row = result.fetchone()
        if not row:
        ...
        log.exception("patch_job failed: %s", e)
        raise HTTPException(status_code=500, detail=f"patch_job failed: {type(e).__name__}: {e}")
